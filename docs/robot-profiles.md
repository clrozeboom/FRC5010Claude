...
pose estimation (`VisionFactory`) and field-geometry helpers (`FieldConstants`) all follow your
choice — they never load their own copy.

```java
@Override
public AprilTagFieldLayout getAprilTagFieldLayout() {
    // Default (if not overridden) is AprilTagFields.kDefaultField (welded).
    return AprilTagFieldLayout.loadField(AprilTagFields.k2026RebuiltAndymark); // we play AndyMark
}
```

`RebuiltRobotProfile` does exactly this. For a practice/lab layout you can also call
`AprilTags.setAprilTagFieldLayout(...)` directly during init — but do it before anything reads
field constants (the container's profile call already runs ahead of the subsystems).
