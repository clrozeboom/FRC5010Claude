...
> This lesson *replaces* the library's built-in demo intake with your own version. The finished
> reference code lives under `src/main/java/frc/robot/` (the package `frc.robot`) and
> `src/main/java/frc/robot/subsystems/`. If you get stuck, peek there — but try it yourself first.
...
3. **Run the tests:** `./gradlew test`. There's a sample `LessonRobotSmokeTest` showing how to
...
The complete, working reference solution is committed in this repo:
