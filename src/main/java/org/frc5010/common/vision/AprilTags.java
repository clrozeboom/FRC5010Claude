...
/**
 * Single shared holder for the active AprilTag field layout, so field-geometry helpers
 * (such as {@link frc.robot.rebuilt.FieldConstants}) and {@code VisionFactory} all resolve
 * tag poses against <b>one</b> layout and never diverge. Mirrors the source Rebuilt2026
 * robot's {@code AprilTags} holder so ported field constants line up tag-for-tag.
 *
 ...
 * builds the drive and vision, so this holder reflects the profile's chosen variant (e.g.
 * {@code RebuiltRobotProfile} selects the AndyMark field). The static default below is only a
 * fallback for code that runs without a profile (some unit tests): WPILib's current-season
 * default ({@code kDefaultField}).
 *
 * <pre>
